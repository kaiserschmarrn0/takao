...
// Description: Checks some CPU things, enables them and provide various functions
...
void check(struct cpucheck *cpucheck)
{
	...
	cpucheck->has_apic = (d & HAS_APIC) != 0;
	cpucheck->has_x2apic = (c & HAS_X2APIC) != 0;
	cpucheck->has_msr = (d & HAS_MSR) != 0;
	cpucheck->has_sse = (d & HAS_SSE) != 0;
	cpucheck->has_sse2 = (d & HAS_SSE2) != 0;
	
	a = 0x80000001;
	__asm__("cpuid" : "=d"(d), "=b"(b), "=c"(c), "+a"(a));
	cpucheck->has_ia32_efer = d & HAS_IA32_EFER;
	
	// If SSE is present, enable it
	if(cpucheck->has_sse || cpucheck->has_sse2) {
