...
EFI_DEVICE_PATH *AppendDevicePathNode(IN EFI_DEVICE_PATH *Src1, IN EFI_DEVICE_PATH *Src2)
// Src1 may have multiple "instances" and each instance is appended
...
EFI_DEVICE_PATH *UnpackDevicePath(IN EFI_DEVICE_PATH *DevPath)
{
...
EFI_DEVICE_PATH* AppendDevicePathInstance(IN EFI_DEVICE_PATH *Src, IN EFI_DEVICE_PATH *Instance)
{
	UINT8           *Ptr;
...
EFI_STATUS LibDevicePathToInterface(IN EFI_GUID *Protocol, IN EFI_DEVICE_PATH *FilePath,
									OUT VOID **Interface)
{
	EFI_STATUS              Status;
	EFI_HANDLE              Device;
...
static VOID _DevPathPci(IN OUT POOL_PRINT *Str, IN VOID *DevPath)
{
	PCI_DEVICE_PATH         *Pci;
...
static VOID _DevPathPccard(IN OUT POOL_PRINT *Str,IN VOID *DevPath)
{
...
static VOID _DevPathMemMap(IN OUT POOL_PRINT *Str,IN VOID *DevPath)
{
...
static VOID _DevPathController (IN OUT POOL_PRINT *Str,IN VOID *DevPath)
{
...
static VOID _DevPathVendor(IN OUT POOL_PRINT *Str,IN VOID *DevPath)
{
 ...
 */
static VOID _DevPathAcpi(IN OUT POOL_PRINT *Str, IN VOID *DevPath)
{
...
static VOID _DevPathAtapi(IN OUT POOL_PRINT *Str,IN VOID *DevPath)
{
...
static VOID _DevPathScsi(IN OUT POOL_PRINT *Str, IN VOID *DevPath)
{
...
static VOID _DevPathFibre(IN OUT POOL_PRINT *Str, IN VOID *DevPath)
{
...
static VOID _DevPath1394(IN OUT POOL_PRINT *Str, IN VOID *DevPath)
{
...
static VOID _DevPathUsb(IN OUT POOL_PRINT *Str, IN VOID *DevPath)
{
	USB_DEVICE_PATH         *Usb;
...
static VOID _DevPathI2O(IN OUT POOL_PRINT *Str, IN VOID *DevPath)
{
	I2O_DEVICE_PATH         *I2O;
...
static VOID _DevPathMacAddr (IN OUT POOL_PRINT *Str, IN VOID *DevPath)
{
...
static VOID CatPrintIPv4(IN OUT POOL_PRINT * Str , IN EFI_IPv4_ADDRESS * Address)
{
	CatPrint( Str , L"%d.%d.%d.%d" , Address-> Addr[ 0 ] , Address-> Addr[ 1 ] ,
		Address-> Addr[ 2 ] , Address-> Addr[ 3 ] ) ;
}

static BOOLEAN IsNotNullIPv4(IN EFI_IPv4_ADDRESS * Address)
{
	UINT8 val ;
...
static VOID CatPrintNetworkProtocol(IN OUT POOL_PRINT * Str , IN UINT16 Proto)
{
	if ( Proto == 6 ) {
		CatPrint( Str , L"TCP" ) ;
	} else if ( Proto == 17 ) {
